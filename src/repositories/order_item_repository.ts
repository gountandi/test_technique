...
import { CreateOrderItemDto, OrderItemDto, UpdateOrderItemDto } from "../types/order_item.js";
...
export default class OrderItemRepository {
    public async create(data: CreateOrderItemDto): Promise<OrderItemDto> {
        ...
        if (!menu) throw new ResourceNotFoundError(`Menu ${data.menuId} introuvable`);
    ...
    public async findByOrderAndMenu(orderId: number, menuId: number): Promise<OrderItemDto | null> {
        return prisma.orderItem.findFirst({ where: { orderId, menuId } });
    }
}